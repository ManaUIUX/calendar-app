...
const MARK_NONE_ID = "icon07";
const MARKS = [
  { id: "icon01", icon: "img/icon01.svg", label: "マーク1" },
  { id: "icon02", icon: "img/icon02.svg", label: "マーク2" },
  { id: "icon03", icon: "img/icon03.svg", label: "マーク3" },
  { id: "icon04", icon: "img/icon04.svg", label: "マーク4" },
  { id: "icon05", icon: "img/icon05.svg", label: "マーク5" },
  { id: "icon06", icon: "img/icon06.svg", label: "マーク6" },
  { id: "icon07", icon: "img/icon07.svg", label: "なし" },
];
...
const markImgCache = {};
function loadMarkImage(mark) {
  const cached = markImgCache[mark.id];
  if (cached && cached.complete && cached.naturalWidth) {
    return Promise.resolve(cached);
  }
  const src = MARK_ICON_DATA[mark.id];
  if (!src) {
    return Promise.reject(new Error(`no icon data: ${mark.id}`));
  }
  return new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
      markImgCache[mark.id] = img;
      resolve(img);
    };
    img.onerror = () => reject(new Error(`mark image load failed: ${mark.id}`));
    img.src = src;
  });
}
const CHIPS = [
...
const normalizeMarkId = (id) => {
  if (!id) return MARKS[0].id;
  if (MARKS.some((m) => m.id === id)) return id;
  return LEGACY_MARK_MAP[id] || MARKS[0].id;
};
const isMarkVisible = (id) => normalizeMarkId(id) !== MARK_NONE_ID;
const markIconSrc = (id) => {
  const m = MARKS.find((item) => item.id === normalizeMarkId(id));
  return m ? m.icon : MARKS[0].icon;
};
const hasDayMark = (data) => data && isMarkVisible(data.markId);
const ensureMarkImagesLoaded = () => {
  if (typeof MARK_ICON_DATA === "undefined") {
    return Promise.reject(new Error("mark-icons.js is not loaded"));
  }
  return Promise.all(MARKS.map((m) => loadMarkImage(m)));
};
...
function renderMarks() {
  document.getElementById("markRow").innerHTML = MARKS.map(
    (m) => `
      <button type="button" class="mark-btn${S.mark === m.id ? " active" : ""}" onclick="setMark('${m.id}')" title="${m.label}">
        <img src="${m.icon}" alt="" width="24" height="24" class="mark-btn__icon">
      </button>`,
  ).join("");
}
const setMark = (id) => {
...
function updateSel() {
  ...
  if (isMarkVisible(m.id)) {
    const img = document.createElement("img");
    img.src = m.icon;
    img.alt = "";
    img.className = "sel-dot__icon";
...
function renderCalendar() {
  ...
  for (let d = 1; d <= total; d++) {
    ...
    if (data) {
      const hasMark = hasDayMark(data);
      const hasLabel = !!data.label;
      if (hasMark || hasLabel) {
        const wrap = document.createElement("div");
        wrap.className =
          "d-mark-wrap" + (hasMark ? "" : " d-mark-wrap--text-only");
        if (hasMark) {
          const img = document.createElement("img");
          img.src = markIconSrc(data.markId);
          img.alt = "";
          img.className = "d-mark__icon";
    ...
    el.onclick = () => {
      if (S.days[key]) delete S.days[key];
        ...
        const txt = S.chip === "自由入力" ? S.customText : S.chip;
        S.days[key] = { label: txt, markId: S.mark };
      }
      renderCalendar();
      save();
    };
...
function wrapLabelLines(ctx, label, maxW) {
  const chars = [...label];
  let line = "";
  const lines = [];
...
function measureLabelLines(ctx, label, lsz, lineH, maxTextW, fb) {
  ctx.font = `600 ${lsz}px ${fb}`;
  const lines = wrapLabelLines(ctx, String(label), maxTextW);
  return { lines, height: lines.length * lineH };
}

function calcMarkLayout(ctx, fb, opts) {
  const { colW, cellW, contentH, label, hasMark, hasLabel, scale } = opts;
  const maxTextW = cellW * 0.95;
  const minMark = Math.round(10 * scale);
  const minLsz = Math.round(9 * scale);
  let markSize = hasMark ? Math.round(colW * 0.55) : 0;
  let lsz =
    hasLabel && label
      ? label.length <= 2
        ? Math.round(colW * 0.17)
        : label.length <= 6
  ...
  if (hasLabel && label) {
    ({ lines: labelLines, height: labelH } = measureLabelLines(
      ctx,
      label,
      lsz,
      lineH,
  ...
  let blockH = Math.max(markSize, labelH);

  const shrink = (factor) => {
    if (hasMark) markSize = Math.max(Math.round(markSize * factor), minMark);
    if (hasLabel) {
      lsz = Math.max(Math.round(lsz * factor), minLsz);
      lineH = lsz * 1.2;
      ({ lines: labelLines, height: labelH } = measureLabelLines(
        ctx,
        label,
    ...
    blockH = Math.max(markSize, labelH);
  };

  if (contentH > 0 && blockH > contentH) {
    shrink(contentH / blockH);
    if (blockH > contentH) shrink(contentH / blockH);
  }

  return { markSize, lsz, lineH, labelLines, blockH };
}

function drawDayLabelOnMark(
  ctx,
  cx,
  centerY,
  label,
  ...
  const labelLines = lines || wrapLabelLines(ctx, String(label), maxTextW);
  const totalH = labelLines.length * lineH;
  const startY = centerY - totalH / 2 + lineH / 2;
  labelLines.forEach((ln, i) => ctx.fillText(ln, cx, startY + i * lineH));
}

function drawDayMark(ctx, cx, y, markId, markSize) {
  const id = normalizeMarkId(markId);
  if (!isMarkVisible(id)) return;
  const img = markImgCache[id];
...
function drawCalendarToCanvas(canvas, W, H, tszBase) {
  ...
  const titleTop = Math.round(42 * scale);
  const titleToDowGap = Math.round(28 * scale);
  const y0 = titleTop + titleNumFsz + titleToDowGap;
  const colW = (W - pad * 2) / 7;
  const dowH = colW * 0.46;
  ...
  const footerSafetyGap = Math.round(20 * scale);
  const footerReserve =
    footerLines.length > 0
      ? bottomMargin +
        (footerLines.length - 1) * lineGap +
        footerFsz +
        footerSafetyGap
      : Math.round(36 * scale);
  const availH = H - y0 - footerReserve;
  const rowH = (availH - dowH) / numRows;
  ...
  const numDotGap = Math.round(20 * scale);
  const numFsz = Math.round(colW * 0.18);
  const cellPadY = Math.round(4 * scale);
  ...
  for (let d = 1; d <= total; d++) {
    ...
    const hasContent = hasDot || hasLabel;
    ...
    if (hasContent) {
      const dateBaseline = cellY + cellPadY + numFsz;
      ctx.fillText(d, cx, dateBaseline);

      const contentTop = dateBaseline + numDotGap;
      const contentBottom = cellY + cellH - cellPadY;
      const contentH = Math.max(0, contentBottom - contentTop);
      const layout = calcMarkLayout(ctx, fb, {
        colW,
        cellW,
        contentH,
        label: data.label,
      ...
      const markY = centerY - layout.markSize / 2;

      if (hasDot) drawDayMark(ctx, cx, markY, data.markId, layout.markSize);
      if (hasLabel) {
        drawDayLabelOnMark(
          ctx,
          cx,
      ...
      ctx.fillText(d, cx, cellY + cellH / 2 + numFsz / 3);
    }
  }
  ...
  requestAnimationFrame(() => {
    ensureMarkImagesLoaded()
      .then(() => {
        ...
        const [W, H] = sizes[S.imgSize] || sizes.feed;
...
document.getElementById("customInput").addEventListener("input", (e) => {
    S.customText = e.target.value.slice(0, 12);
