...
const hasDayMark = (data) => data && data.dotHex && data.dotHex !== "transparent";

function applyTheme() {
  if (!THEMES.some((t) => t.id === S.theme)) S.theme = THEMES[0].id;
  const app = document.getElementById("app");
  app.className = "app theme-" + S.theme;
  app.style.setProperty("--font", S.font);
...
function printCalendar() {
  // A4縦・約300dpi相当のサイズで描画して印刷
  const W = 2480,
    H = 3508;
  const canvas = document.createElement("canvas");
  drawCalendarToCanvas(canvas, W, H, 151);
